...
export class RecordingService {
    ...
    async getAccessToken() {
        return await getAccessToken(REFRESH_TOKEN, CLIENT_ID, CLIENT_SECRET, axios)
    }

    async getMessages(access_token) {
        const messagesResponse = await axios.get(`https://www.googleapis.com/gmail/v1/users/me/messages?q=from:${MAIL_AUTHOR}`, {
            headers: {
